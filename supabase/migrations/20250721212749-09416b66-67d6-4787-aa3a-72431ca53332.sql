-- Check and remove ANY remaining check constraints on protests table
DO $$ 
DECLARE 
...
BEGIN
    FOR constraint_name IN 
        SELECT conname 
        FROM pg_constraint 
        WHERE conrelid = 'public.protests'::regclass 
        AND contype = 'c'
    LOOP
        EXECUTE 'ALTER TABLE public.protests DROP CONSTRAINT IF EXISTS ' || constraint_name;
    END LOOP;
END $$;
