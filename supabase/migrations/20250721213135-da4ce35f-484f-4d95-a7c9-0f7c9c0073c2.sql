-- Make property_id nullable so we can insert test data without foreign key issues
ALTER TABLE public.protests ALTER COLUMN property_id DROP NOT NULL;

-- Drop the foreign key constraint 
ALTER TABLE public.protests DROP CONSTRAINT fk_appeal_status_property;

-- Insert mock data with NULL property_id to avoid foreign key issues
INSERT INTO public.protests (
  property_id,
  situs_address,
  owner_name,
  county,
  exemption_status,
  hearing_date,
  assessed_value,
  market_value,
  protest_amount,
  savings_amount,
  tax_year,
  protest_date,
  recommendation,
  documents_generated,
  evidence_packet_url,
  offer_received_date
) VALUES 
(
  NULL,
  '123 Main Street, Austin, TX 78701',
  'John Smith',
  'Travis County',
  'pending',
  '2024-03-15 10:00:00-05',
  450000,
  420000,
  30000,
  2100,
  2024,
  '2024-01-15 09:00:00-06',
  'Accept - Fair market adjustment based on comparable sales',
  true,
  '/documents/evidence-packet-001.pdf',
  '2024-02-20 14:30:00-06'
),
(
  NULL,
  '456 Oak Avenue, Austin, TX 78702',
  'Sarah Johnson',
  'Travis County',
  'pending',
  '2024-04-10 14:00:00-05',
  325000,
  305000,
  20000,
  1400,
  2024,
  '2024-01-20 11:00:00-06',
  'Reject - Offer too low, request higher reduction',
  false,
  NULL,
  '2024-02-25 10:15:00-06'
),
(
  NULL,
  '789 Pine Street, Austin, TX 78703',
  'Michael Brown',
  'Travis County',
  'approved',
  '2024-02-28 09:30:00-06',
  675000,
  640000,
  35000,
  2450,
  2024,
  '2024-01-10 08:00:00-06',
  'Accept - Good settlement offer',
  true,
  '/documents/evidence-packet-002.pdf',
  '2024-02-15 16:45:00-06'
);